...
describe('verifyStripeWebhookSignature', () => {
  ...
  it('accepts one valid signature when multiple v1 signatures are present', () => {
    const valid = signPayload(payload, secret);
    const mixed = `${valid},v1=deadbeef`;
    expect(verifyStripeWebhookSignature(Buffer.from(payload), mixed, secret)).toBe(true);
  });
